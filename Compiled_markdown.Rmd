...
phylo_bold <- bold_seq(taxon = NULL, ids = phylo$processid, bin = phylo$bin_uri, container = NULL, institutions = NULL, 
                  researchers = NULL, geo = NULL, marker = "COI-5P", response = FALSE)
#I believe the above code ran ok, but we have 115 rows instead of 124 now. So we will double check. 
```
...
```
Now "phylo_bold_trim" is trimmed of unnecessary columns, in proper format. The final goal for this object is to read into a fasta file. Before we do that, we need to combine the "ID" and "name" columns into one column. After doing this, we check to see what class our object is. We need it to be one dataframe. 
```{r}
