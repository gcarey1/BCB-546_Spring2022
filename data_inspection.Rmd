...
First we make a new, empty column in which to add the retrieved sequences. 
```{r}
...
```{r}
seqlist2.1<- as.data.frame(matrix(unlist(sequence_df2, use.names = FALSE), ncol = 3, byrow= TRUE))
```
...
#sequence_merge_main <- merge(seqlist2, bdj, by = "id")
```
...
```{r}
#seqlist3df <- as.data.frame(IDs = seqlist3$processid, names = seqlist3$names, sequences = eqlist3$retrieved_seq)
#class(seqlist3)
#names <- (seqlist3$names)
#sequences <- (seqlist3$retrieved_seq)
#dfseqlist3 <- data.frame(IDs,names,sequences)

#seqlist3_df2 <- as.tibble(seqlist3)
#df_seqlist3.fasta = dataframe2fas(seqlist3_df2, file = "df_seqlist3.fasta")
seqlist3combo_df <- as.data.frame(seqlist3combo)
...
mySequences1
```
These sequences appear to be correctly aligned. 